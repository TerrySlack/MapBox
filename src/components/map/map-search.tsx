    ...
    const searchLocations = async () => {
      ...
      try {
        const res = await fetch(
          ...
          }&limit=5&proximity=-122.4194,37.7749` //&country=US
        );
  ...
  const handleSelect = async (suggestion: LocationSuggestion) => {
    try {
      ...
      if (map && featuresData?.length > 0) {
        ...
        map.flyTo({
          center: coordinates,
          zoom: 16, // Slightly closer for a "Google Maps feel"
          bearing: 0, // or maybe a slight turn like 10-30
          pitch: 45, // adds 3D tilt
          speed: 1.2, // slower = smoother
          curve: 1.8, // adjust how "dramatic" the fly effect feels
